...
function stateFor(progress: Progress, index: number): LevelState {
  const level = DONJON_LEVELS[index]
  if (progress.completedLessons[level.id]) return 'done'
  if (index === 0) return 'unlocked'
  const previous = DONJON_LEVELS[index - 1]
  return progress.completedLessons[previous.id] ? 'unlocked' : 'locked'
}

export default function DonjonScreen({ progress, onSelectLevel }: DonjonScreenProps) {
  const doneCount = DONJON_LEVELS.filter((l) => progress.completedLessons[l.id]).length

  const showChapterAt = DONJON_LEVELS.map(
    (level, index) => index === 0 || level.chapter !== DONJON_LEVELS[index - 1].chapter,
  )

  return (
    <div className="mx-auto max-w-md px-4 pb-10 pt-5">
      <div className="mb-6">
        <h1 className="text-xl font-extrabold text-white">Le Donjon 🏰</h1>
        <p className="mt-0.5 text-sm text-white/50">
          {doneCount} / {DONJON_LEVELS.length} niveaux terminés — un robot, un donjon, tes propres
          fonctions.
        </p>
      </div>

      <div className="flex flex-col gap-2">
        {DONJON_LEVELS.map((level, index) => {
          const state = stateFor(progress, index)
          const showChapter = showChapterAt[index]
          return (
            <div key={level.id}>
              {showChapter && (
                <p className="mb-2 mt-5 first:mt-0 text-xs font-bold uppercase tracking-wide text-indigo-300/70">
                  {level.chapter}
                </p>
              )}
              <button
