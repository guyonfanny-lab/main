import type { Difficulty, Progress, Project } from '../types'

interface ProjectStepsScreenProps {
  project: Project
  progress: Progress
...
function stateFor(progress: Progress, project: Project, index: number): StepState {
...
export default function ProjectStepsScreen({
  ...
  return (
    <div className="mx-auto max-w-md px-4 pb-10 pt-5">
      ...
      <div className={`mb-6 rounded-2xl bg-gradient-to-r ${project.color} p-4 shadow-lg`}>
        <div className="mb-1 flex items-center justify-between">
          <span className="text-3xl">{project.emoji}</span>
          <span
            className={`rounded-full px-2.5 py-1 text-[11px] font-bold ${DIFFICULTY_STYLE[project.difficulty]}`}
          >
            {project.difficulty}
          </span>
        </div>
        ...
        <p className="mt-2 text-xs font-semibold text-white/70">
          {doneCount} / {project.steps.length} étapes terminées
        </p>
      </div>

      <div className="flex flex-col gap-3">
        {project.steps.map((step, index) => {
          const state = stateFor(progress, project, index)
          return (
            <button
              key={step.id}
              type="button"
